(function($) {
    $(document).ready(function() {
        ...
        $('body').on('click', '#vce-add-module', function(e) {
            e.preventDefault();
            tb_show('Add Module', '#TB_inline?width=600&height=800&inlineId=vce-hidden-module');
        });

        /* Add new blank module */
        $('body').on('click', '#vce-add-module-blank', function(e) {
            e.preventDefault();
            tb_show('Add Module', '#TB_inline?width=600&height=800&inlineId=vce-hidden-module-blank');
        });

        /* Add new cpt module */
        $('body').on('click', '.vce-add-module-cpt', function(e) {
            e.preventDefault();
            var cpt_type = $(this).data('type');
            tb_show('Add Module', '#TB_inline?width=600&height=800&inlineId=vce-hidden-module-' + cpt_type);
        });

        /* Add new category module */
        $('body').on('click', '#vce-add-module-cat', function(e) {
            e.preventDefault();
            tb_show('Add Module', '#TB_inline?width=600&height=800&inlineId=vce-hidden-module-cat');
        });
        ...
        $('body').on('click', '.vce-edit-module', function(e) {
            ...
            tb_show('Edit Module - ' + module_name, '#TB_inline?width=600&height=800&inlineId=vce-hidden-module-' + div_id);
        });
        ...
        $('body').on('click', '.vce-deactivate-module', function(e) {
            ...
            var h_data = parent.find('.vce-deactivate').val();
            _self.find('span').toggleClass('vce-hidden');
            if (h_data == 1) {
                parent.find('.vce-deactivate').val('0');
                parent.addClass('vce-module-disabled');
            } else {
                parent.find('.vce-deactivate').val('1');
                parent.removeClass('vce-module-disabled');
            }
        ...
        $('body').on('click', '.vce-remove-module', function(e) {
            e.preventDefault();
            var remove_module = confirm("Are you sure?");
            if (remove_module) { 
        ...
        $('body').on('click', 'a.vce-save-module', function(e) {
            ...
            if ($clean_form.hasClass('edit')) {
                ...
                $('#vce-modules-wrap').append('<li data-module="' + count + '"><span class="module-title">' + mod_title + '</span> <span class="actions"> <a href="#" class="vce-edit-module">Edit</a> |'+ 
                    ' <a href="#" class="vce-deactivate-module"><span class="vce-hidden">Activate</span><span>Deactivate</span>  </a> |'+
                    '<a href="#" class="vce-remove-module">Remove</a></span><div id="vce-hidden-module-' + count + '" class="vce-hidden-module"><div class="vce-module-form edit">' + $clean_form.html() + '</div></div></li>');
        ...
        /* Make option items sortable */
        $(".vce-sortable-items").sortable({
            revert: false,
            cursor: "move",
            placeholder: "vce-item-placeholder"
        });
