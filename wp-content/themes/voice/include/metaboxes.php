...
?>
	   ...
	   <div class="vce-opt-item">
			...
			<input type="text" name="vce[fa_author]" value="<?php echo esc_attr(vce_get_authors_username_by_id($vce_meta['fa_author'])); ?>" class="herald-count-me" />
			<br>
	  		<label><input type="radio" name="vce[fa_author_inc_exc]" value="in" <?php checked( $vce_meta['fa_author_inc_exc'], 'in' ); ?> class="vce-count-me" /><?php esc_html_e('Include', THEME_SLUG) ?></label><br/>
	   		<label><input type="radio" name="vce[fa_author_inc_exc]" value="not_in" <?php checked( $vce_meta['fa_author_inc_exc'], 'not_in' ); ?> class="vce-count-me" /><?php esc_html_e('Exclude', THEME_SLUG) ?></label><br/>
			...
			<?php  $ids = !empty( $vce_meta['fa_exclude_by_id'] ) ?  implode(', ', $vce_meta['fa_exclude_by_id']) : ''; ?>
	   		<input type="text" name="vce[fa_exclude_by_id]" value="<?php echo esc_attr( $ids ); ?>" class="vce-count-me"/>
	   ...
	   <div class="vce-opt-item">
		   ...
		   <?php endforeach; ?>
			...
			<label><input type="radio" name="vce[fa_sort]" value="ASC" <?php checked( $vce_meta['fa_sort'], 'ASC' ); ?> class="vce-count-me" /><?php esc_html_e('Ascending', THEME_SLUG) ?></label><br/><br>
...
?>
		...
		<ul id="vce-modules-wrap">
			...
			<?php foreach ( $vce_meta['modules'] as $i => $module ) : ?>
				...
				<li data-module="<?php echo $i; ?>" class="vce-list <?php echo esc_attr($vce_module_disabled); ?>">
					<span class="module-title"><?php echo $module['title']; ?></span> 
					<span class="actions"> 
						<a href="#" class="vce-edit-module"><?php _e('Edit', THEME_SLUG); ?></a> | 
						<a href="#" class="vce-deactivate-module">
						...
						<a href="#" class="vce-remove-module"><?php _e('Remove', THEME_SLUG); ?></a>
					</span>
		...
		<p>
			<a id="vce-add-module" href="javascript:void(0);" class="button-secondary"><?php _e( 'Add posts module', THEME_SLUG ); ?></a> 
			<a id="vce-add-module-blank" href="javascript:void(0);" class="button-secondary"><?php _e( 'Add html/text module', THEME_SLUG ); ?></a>
			<a id="vce-add-module-cat" href="javascript:void(0);" class="button-secondary"><?php _e( 'Add category module', THEME_SLUG ); ?></a>
			<?php foreach ($module_cpt_def as $cpt): ?>
				<a href="javascript:void(0);" class="button-secondary vce-add-module-cpt" data-type="<?php echo $cpt['type'] ?>">  <?php echo __( 'Add ' .  $cpt['type_name'] . ' module ', THEME_SLUG ) ; ?> </a> 
			<?php endforeach ?>
		</p>
		...
		<?php foreach ($module_cpt_def as $cpt): ?>
			<?php vce_generate_cpt_module_field( $cpt, false, $module_cpt_options[$cpt['type']] ); ?>
		<?php endforeach ?>
