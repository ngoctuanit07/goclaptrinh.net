...
class VCE_Posts_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( !empty( $instance['manual'] ) && !empty( $instance['manual'][0] ) ) {
			...
			if( $instance['exclude_current'] && is_single() ) {
...
?>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['exclude_current'] = isset( $new_instance['exclude_current'] ) ? 1 : 0;
	...
	function form( $instance ) {
		...
		<p>
			<input id="<?php echo $this->get_field_id( 'exclude_current' ); ?>" type="checkbox" name="<?php echo $this->get_field_name( 'exclude_current' ); ?>" value="1" <?php checked( 1, $instance['exclude_current'] ); ?>/>
			<label for="<?php echo $this->get_field_id( 'exclude_current' ); ?>"><?php _e( 'Exclude current post', THEME_SLUG ); ?></label>
			<small class="howto"><?php _e( 'If sidebar is used on single post template, exclude current post from widget', THEME_SLUG ); ?></small>
		</p>
